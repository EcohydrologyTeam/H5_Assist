...
module h5_datasets
  ...
  logical function h5_create_2d_dataset(loc_id, dataset_name, nrows_hdf, ncols_hdf, data_type, chunk_in_time, kind_nbytes, compressed, dataset_id) result(success)
    ...
    if (compressed) then
      ...
      if (chunk_in_time) then 
        ...
        dataset_dims(2) = 1
        max_dims(2)     = H5S_UNLIMITED_F ! unlimited column length
        max_dims(1)     = H5S_UNLIMITED_F ! unlimited row length, added on 3/25/2019
      else
        ...
        dataset_dims(1) = 1
        max_dims(1)     = H5S_UNLIMITED_F ! unlimited row length
        max_dims(2)     = H5S_UNLIMITED_F ! unlimited column length, added on 3/25/2019
      end if
      !
  ...
  subroutine debug_write_compound(h5compound_arr, nrows)
    ...
    do field = 1, size(h5compound_arr)
      write(*,*) "name = " // trim(h5compound_arr(field)%name)
  ...
  ! .....................................................................................................
  !
  ! Add row to 2D HDF5 dataset: Real type
  !
  ! .....................................................................................................
  ! Add a row of data to an existing 2D real dataset
  ! 
  ! .....................................................................................................
  logical function h5_add_row_2d_dataset_r(dataset_id, hdf_array) result(success)
    !DEC$ ATTRIBUTES STDCALL, REFERENCE, DLLEXPORT, ALIAS:"h5_add_row_2d_dataset_r" :: h5_add_row_2d_dataset_r
    !
    integer(hid_t), intent(in)          :: dataset_id     ! Identifier of open dataset
    real(kind=c_float), &
      dimension(:), &
      intent(inout)                     :: hdf_array      ! Data array to write and return (must first be allocated by calling function)
    integer                             :: startrow       ! Index of next row to write (nrows_hdf + 1)
    integer                             :: startcol = 0   ! Index of first column of subset
    integer :: nrows_subset = 1
    integer(hsize_t), allocatable, &
      dimension(:)                      :: data_dims      ! Dataset dimensions
    integer(hsize_t), allocatable, &
      dimension(:)                      :: max_dims       ! Maximum dataset dimensions
    integer(hid_t)                      :: dataspace_id   ! Data space identifier    
    integer                             :: nrows_hdf      ! Number of rows in existing dataset
    integer                             :: ncols_hdf      ! Number of columns in existing dataset
    integer                             :: rank = 2       ! Rank (number of dimensions) of dataset
    integer                             :: hdf_error      ! HDF5 error flag
    logical                             :: is_ok          ! Did the function run without error?
    !
    success = .false.
    !
    ! Get size of current dataset on disk (nrows_hdf x ncols_hdf)
    !
    ! Get data space from dataset
    call h5dget_space_f(dataset_id, dataspace_id, hdf_error)
    if (hdf_error == -1) return
    !
    !call h5sget_simple_extent_ndims_f(dataspace_id, rank, hdf_error);
    !
    ! Get dataset dimensions, data_dims
    call h5sget_simple_extent_dims_f(dataspace_id, data_dims, max_dims, hdf_error)
    if (hdf_error == -1) return
    !
    nrows_hdf = 1
    ncols_hdf = size(hdf_array)
    !
    startrow = data_dims(2)
    !
    ! Write row to dataset (dataset will be expanded by this function before writing)
    is_ok = h5_write_dataset_2d_array_r(dataset_id, startrow, startcol, 1, ncols_hdf, hdf_array)
    if (hdf_error == -1) return
    !
    success = .true.
  end function
