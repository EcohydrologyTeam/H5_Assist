...
namespace C2F_Tests
{
  [Trait("xunit: All", "")]
  [Trait("xunit: C2F All", "")]
  [Trait("xunit: C2F Datasets", "")]
  public class C2F_Tests_Datasets_xunit
  {
    // ----- Dataset Tests -----
    ...
    public C2F_Tests_Datasets_xunit()
    {
    ...
    public void C2F_Test_h5_get_dataset_dimensions()
    {
      ...
      Assert.Equal(ncols_hdf, data_dims[0]);
      Assert.Equal(nrows_hdf, data_dims[1]);
    ...
    public void C2F_Test_h5_write_dataset_2d_array_r_BY_ROW()
    {
      ...
      for (index = 0; index < nrows_hdf; index++) {
        ...
        for (int j = 0; j < ncols_hdf; j++) {
          Assert.Equal(hdf_array_in[0, j], hdf_array_out[0, j]);
        }
      }
